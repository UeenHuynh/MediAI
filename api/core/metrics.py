...
class MetricsCollector:
    ...
    def record_prediction_latency(self, prediction_type: str, latency_ms: float):
        ...
        if prediction_type in self.prediction_latencies:
            ...
            if len(self.prediction_latencies[prediction_type]) > 1000:
                self.prediction_latencies[prediction_type] = self.prediction_latencies[
                    prediction_type
                ][-500:]
    ...
    def to_prometheus_format(self) -> str:
        ...
        for pred_type in ["sepsis", "mortality"]:
            ...
            lines.append(
                f'mediai_prediction_latency_ms{{type="{pred_type}",quantile="0.5"}} {lat["p50"]}'
            )
            lines.append(
                f'mediai_prediction_latency_ms{{type="{pred_type}",quantile="0.95"}} {lat["p95"]}'
            )
...
def record_prediction(
    prediction_type: str, risk_score: float, latency_ms: float = None
):
