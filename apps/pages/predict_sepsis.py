...
def show_sepsis_prediction():
    ...
    st.markdown(
        '<div class="page-header">🔬 Sepsis Risk Prediction</div>',
...
def show_demographics_vitals():
    ...
    with col2:
        age = st.number_input(
            "Age (years)*", min_value=18, max_value=120, value=65, key="age"
        )
    with col3:
        gender = st.selectbox("Gender*", ["M", "F"], key="gender")

    bmi = st.number_input(
        "BMI*", min_value=10.0, max_value=60.0, value=25.5, step=0.1, key="bmi"
    )
    ...
    with col1:
        heart_rate = st.number_input(
            "Heart Rate (bpm)*", min_value=0, max_value=300, value=95, key="hr"
        )
        sbp = st.number_input(
            "Systolic BP (mmHg)*", min_value=40, max_value=250, value=120, key="sbp"
        )

    with col2:
        dbp = st.number_input(
            "Diastolic BP (mmHg)*", min_value=20, max_value=150, value=80, key="dbp"
        )
    ...
    with col3:
        respiratory_rate = st.number_input(
            "Respiratory Rate*", min_value=0, max_value=60, value=16, key="rr"
        )
        spo2 = st.number_input(
            "SpO2 (%)", min_value=0, max_value=100, value=98, key="spo2"
        )
...
def show_laboratory_values():
    ...
    with col2:
        ...
        ph = st.number_input(
            "pH", min_value=6.8, max_value=8.0, value=7.4, step=0.01, key="ph"
        )
...
def show_sofa_scores():
    ...
    with col1:
        respiratory_sofa = st.select_slider(
            "Respiratory", options=[0, 1, 2, 3, 4], value=0, key="sofa_resp"
        )
        cardiovascular_sofa = st.select_slider(
            "Cardiovascular", options=[0, 1, 2, 3, 4], value=0, key="sofa_cardio"
        )

    with col2:
        hepatic_sofa = st.select_slider(
            "Hepatic", options=[0, 1, 2, 3, 4], value=0, key="sofa_hepatic"
        )
        coagulation_sofa = st.select_slider(
            "Coagulation", options=[0, 1, 2, 3, 4], value=0, key="sofa_coag"
        )

    with col3:
        renal_sofa = st.select_slider(
            "Renal", options=[0, 1, 2, 3, 4], value=0, key="sofa_renal"
        )
        neurological_sofa = st.select_slider(
            "Neurological (GCS)", options=[0, 1, 2, 3, 4], value=0, key="sofa_neuro"
        )
...
def show_trends_time():
    ...
    st.info(
        "Enter the change in value over the specified time period (positive = increase, negative = decrease)"
    )
...
def handle_prediction():
    ...
    request_data = {
        ...
        "features": {
            ...
            "albumin": (
                st.session_state.albumin if st.session_state.albumin > 0 else None
            ),
        ...
        try:
            ...
            try:
                response = requests.post(
                    f"{API_URL}/api/v1/predict/sepsis", json=request_data, timeout=30
                )
