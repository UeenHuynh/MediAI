...
def show():
    ...
    with col2:
        ...
        if st.button("🔬 New Sepsis Prediction", use_container_width=True):
            st.session_state.current_page = "🔬 Predict Sepsis"
            st.rerun()

        if st.button("💔 New Mortality Prediction", use_container_width=True):
            st.session_state.current_page = "💔 Predict Mortality"
            st.rerun()
