...
def show_dashboard():
    ...
    st.markdown(
        '<div class="page-header">🏠 ICU Patient Monitoring Dashboard</div>',
...
def show_patient_detail(patient: pd.Series):
    ...
    with col2:
        ...
        st.metric(
            "Heart Rate",
            f"{patient['hr']} bpm",
            delta=None if patient["hr"] < 100 else "High",
        )
        st.metric("Blood Pressure", f"{patient['bp_sys']}/{patient['bp_dia']} mmHg")
        st.metric(
            "SpO2",
            f"{patient['spo2']}%",
            delta=None if patient["spo2"] >= 95 else "Low",
        )
        st.metric(
            "Temperature",
            f"{patient['temp']}°C",
            delta=None if patient["temp"] < 38 else "Fever",
        )
        st.metric(
            "Respiratory Rate",
            f"{patient['rr']}/min",
            delta=None if patient["rr"] < 20 else "High",
        )
    ...
    with col3:
        ...
        st.metric(
            "WBC",
            f"{patient['wbc']} ×10⁹/L",
            delta=None if patient["wbc"] < 12 else "High",
        )
        st.metric(
            "Lactate",
            f"{patient['lactate']} mmol/L",
            delta=None if patient["lactate"] < 2 else "High",
        )
        st.metric(
            "Creatinine",
            f"{patient['creatinine']} mg/dL",
            delta=None if patient["creatinine"] < 1.5 else "High",
        )
        st.metric(
            "Platelets",
            f"{patient['platelets']} ×10⁹/L",
            delta=None if patient["platelets"] > 150 else "Low",
        )
        st.metric(
            "Bilirubin",
            f"{patient['bilirubin']} mg/dL",
            delta=None if patient["bilirubin"] < 1.5 else "High",
        )
    ...
    with col1:
        ...
        st.markdown(
            f"<h2 style='color: {sepsis_color};'>{patient['sepsis_score']:.1%}</h2>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"**Risk Level:** <span style='color: {sepsis_color}; font-weight: bold;'>{patient['sepsis_risk']}</span>",
            unsafe_allow_html=True,
        )
    ...
    with col2:
        ...
        st.markdown(
            f"<h2 style='color: {mortality_color};'>{patient['mortality_score']:.1%}</h2>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"**Risk Level:** <span style='color: {mortality_color}; font-weight: bold;'>{patient['mortality_risk']}</span>",
            unsafe_allow_html=True,
        )
...
def show_risk_trend_chart():
    ...
    fig.add_trace(
        go.Scatter(
            x=hours, y=high, name="High", fill="tonexty", line=dict(color="#ef4444")
        )
    )
    fig.add_trace(
        go.Scatter(
            x=hours, y=medium, name="Medium", fill="tonexty", line=dict(color="#f59e0b")
        )
    )
    fig.add_trace(
        go.Scatter(
            x=hours, y=low, name="Low", fill="tozeroy", line=dict(color="#10b981")
        )
    )
