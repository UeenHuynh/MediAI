...
def show_mortality_prediction():
    ...
    st.markdown(
        '<div class="page-header">💔 ICU Mortality Risk Prediction</div>',
    ...
    with st.form("mortality_prediction_form"):
        ...
        with col1:
            patient_id = st.text_input(
                "Patient ID*", value=st.session_state.mort_patient_id, key="mort_pid"
            )
        with col2:
            age = st.number_input(
                "Age (years)*", min_value=18, max_value=120, value=65, key="mort_age"
            )
        with col3:
        ...
        st.caption(
            "Enter the WORST (most abnormal) values observed in the first 24 hours of ICU admission"
        )

        col1, col2, col3 = st.columns(3)

        with col1:
            worst_hr = st.number_input(
                "Worst Heart Rate (bpm)",
                min_value=20,
                max_value=250,
                value=100,
                key="mort_hr",
            )
            worst_sbp = st.number_input(
                "Worst SBP Low (mmHg)",
                min_value=40,
                max_value=200,
                value=100,
        ...
        with col2:
            ...
            worst_spo2 = st.number_input(
                "Worst SpO2 (%)", min_value=50, max_value=100, value=95, key="mort_spo2"
            )
            worst_gcs = st.number_input(
                "Worst GCS", min_value=3, max_value=15, value=14, key="mort_gcs"
            )

        with col3:
            worst_lactate = st.number_input(
        ...
        with col1:
            sofa_day1 = st.number_input(
                "SOFA Score (Day 1)",
                min_value=0,
                max_value=24,
                value=3,
                help="Sequential Organ Failure Assessment score on Day 1",
                key="mort_sofa",
            )

        with col2:
            apache_ii = st.number_input(
                "APACHE-II Score",
                min_value=0,
                max_value=71,
                value=10,
                help="Acute Physiology and Chronic Health Evaluation II score",
                key="mort_apache",
            )
        ...
        with col3:
            vasopressor = st.checkbox(
                "Vasopressor Use",
                value=False,
                help="Did patient receive vasopressors (norepinephrine, epinephrine, etc)?",
                key="mort_vaso",
            )
        ...
        with col2:
            submitted = st.form_submit_button(
                "💔 Predict Mortality Risk", use_container_width=True
            )
...
def handle_mortality_prediction():
    ...
    with st.spinner("💔 Analyzing patient mortality risk..."):
        try:
            ...
            # Run prediction
            prediction_result = model_service.predict_mortality(
...
def show_mortality_result(result: dict):
    ...
    with col2:
        ...
        if risk_level == "CRITICAL":
            st.error(
                "🔴 **Very High Risk (>75%)**: Intensive care and family discussion recommended"
            )
        elif risk_level == "HIGH":
            st.warning(
                "🟠 **High Risk (50-75%)**: Close monitoring and treatment escalation needed"
            )
        elif risk_level == "MEDIUM":
            st.warning(
                "🟡 **Medium Risk (20-50%)**: Continue current treatment, monitor closely"
            )
        else:
