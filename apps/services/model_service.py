...
class ModelService:
    ...
    def _prepare_sepsis_features(self, input_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        features["shock_index"] = (
            features["heart_rate"] / features["sbp"] if features["sbp"] > 0 else 0
        )
        features["pulse_pressure"] = features["sbp"] - features["dbp"]
        features["pf_ratio"] = (
            (features["pao2"] / 0.21) if features["spo2"] > 90 else 200
        )  # Simplified
        features["anion_gap"] = features["sodium"] - (
            110 + features["bicarbonate"]
        )  # Simplified
        features["bun_creatinine_ratio"] = (
            features["bun"] / features["creatinine"]
            if features["creatinine"] > 0
            else 15
        )
        features["lactate_albumin_ratio"] = (
            features["lactate"] / features["albumin"]
            if features["albumin"] > 0
            else 0.5
        )
    ...
    def _calculate_sofa_simple(self, features: Dict) -> float:
        ...
        score = 0
        # Respiratory
        if features["pao2"] < 400:
            score += 1
        if features["pao2"] < 300:
            score += 1
        # Cardiovascular
        if features["map"] < 70:
            score += 1
        # Renal
        if features["creatinine"] > 1.2:
            score += 1
        if features["creatinine"] > 2.0:
            score += 1
        # Coagulation
        if features["platelets"] < 150:
            score += 1
        if features["platelets"] < 100:
            score += 1
    ...
    def _calculate_apache_simple(self, features: Dict) -> float:
        ...
        if features["age"] > 65:
            ...
            score += 3
        # Vital signs
        if features["heart_rate"] > 110 or features["heart_rate"] < 55:
            score += 3
        if features["map"] < 70 or features["map"] > 110:
            score += 3
        if features["temperature"] > 38.5 or features["temperature"] < 36:
            score += 2
        if features["respiratory_rate"] > 24:
            score += 2
        # Labs
        if features["sodium"] < 130 or features["sodium"] > 150:
            score += 2
        if features["potassium"] < 3.0 or features["potassium"] > 5.5:
            score += 2
    ...
    def _calculate_sirs(self, features: Dict) -> int:
        ...
        score = 0
        if features["temperature"] > 38 or features["temperature"] < 36:
            score += 1
        if features["heart_rate"] > 90:
            score += 1
        if features["respiratory_rate"] > 20:
            score += 1
        if features["wbc"] > 12 or features["wbc"] < 4:
            score += 1
        return score
    ...
    def _calculate_mews(self, features: Dict) -> int:
        ...
        score = 0
        # Respiratory rate
        if features["respiratory_rate"] >= 30:
            score += 3
        elif features["respiratory_rate"] >= 21:
            score += 2
        elif features["respiratory_rate"] <= 8:
            score += 2
        # Heart rate
        if features["heart_rate"] >= 130:
            score += 3
        elif features["heart_rate"] >= 111:
            score += 2
        elif features["heart_rate"] <= 40:
            score += 2
        # SBP
        if features["sbp"] <= 70:
            score += 3
        elif features["sbp"] <= 80:
            score += 2
        elif features["sbp"] >= 200:
            score += 2
        return score
    ...
    def predict_mortality(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate recommendation
            recommendation = self._generate_mortality_recommendation(
                pred_proba, risk_level
            )
    ...
    def _prepare_mortality_features(self, input_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        features["worst_temperature_high"] = input_data.get(
            "worst_temperature_high", 37.5
        )
        features["worst_respiratory_rate"] = input_data.get(
            "worst_respiratory_rate", 20
        )
        ...
        # Interventions
        features["vasopressor_use"] = (
            1 if input_data.get("vasopressor_use", False) else 0
        )
    ...
    def _generate_mortality_recommendation(
        self, _risk_score: float, risk_level: str
    ) -> str:
